...
This is `armada_nh`, the native NostrHost (`_nh`) package for
[Armada](https://armada.buzz) ([source](https://gitlab.com/soapbox-pub/armada)),
scaffolded from
...
Already filled in for this repo, so a version bump only needs steps 4-5
below: `[app].id` = `armada`, `UPSTREAM_REPO` =
`https://gitlab.com/soapbox-pub/armada.git`, `PACKAGE_ID` = `armada`,
`OUTPUT_DIR` = `dist`. `BUILD_ENV` bakes in `armada_ynh`'s own hardcoded
relay defaults (`VITE_APP_NAME`/`VITE_APP_RELAYS`/`VITE_SEARCH_RELAYS`/
`VITE_PLATFORM_RELAYS`) since this format has no install-time questions —
see the "behavior note" below before changing them. Upstream doesn't
reliably tag releases; `armada_ynh`'s own manifest pins a bare commit hash,
not a tag — `build.sh` handles both, but check for a real tag first.
...
Confirmed live on `nostrhost-clean7`: both the restart and reload cases
broke a sibling app's route (`ditto`) and both self-healed after these
fixes.
